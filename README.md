...
PaperAgent 当前仍在持续迭代，后续版本将继续优化 Agent 协作、文档质量、实验执行、知识管理、交互体验与平台兼容性。欢迎所有使用者通过 Issue 提交问题、使用反馈、功能建议和设计想法，也欢迎开发者直接提交 Pull Request 参与改进。

提交 PR 时，请尽量说明改动目的、实现范围、验证方式及可能影响；涉及界面或文档排版的修改建议附上前后对比截图，涉及新功能或缺陷修复的修改应补充相应测试。较大的架构调整建议先创建 Issue 讨论，以便确认方向并减少重复工作。
